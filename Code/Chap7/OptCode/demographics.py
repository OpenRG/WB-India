...
def get_fert(totpers, min_yr, max_yr, graph=False):
    ...
    pop_file = os.path.join(cur_path, 'data/pop_data.csv')
    ...
    if graph:
        ...
        plt.text(-5, -0.018, 'Source: National Vital Statistics ' +
                 'Reports, Volume 64, Number 1, January 15, 2015.',
                 fontsize=9)
...
def get_mort(totpers, min_yr, max_yr, graph=False):
    ...
    cur_path = os.path.split(os.path.abspath(__file__))[0]
    mort_file = os.path.join(cur_path, 'data/mort_rates2011.csv')
    ...
    if graph:
        ...
        plt.scatter(np.hstack([0, age_year_all]),
                    np.hstack([infmort_rate, mort_rates_all]), s=20,
        ...
        plt.text(-5, -0.2, 'Source: Actuarial Life table, 2011 ' +
                 'Social Security Administration.', fontsize=9)
        plt.tight_layout(rect=(0, 0.03, 1, 1))
...
def get_imm_resid(totpers, min_yr, max_yr, graph=True):
    ...
    if graph:
        ...
        image_path = os.path.join(image_dir, 'imm_rates_orig')
        plt.savefig(image_path)
        plt.show()
        plt.close()
...
def get_pop_objs(E, S, T, min_yr, max_yr, curr_year, GraphDiag=True):
    ...
    imm_rates_mat = \
        np.hstack((np.tile(np.reshape(imm_rates_orig[E:], (S, 1)),
                           (1, fixper)),
                   np.tile(np.reshape(imm_rates_adj[E:], (S, 1)),
                           (1, T + S - fixper))))
